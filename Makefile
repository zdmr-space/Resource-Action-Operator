...
# yusaozdemir.de/resource-action-operator-bundle:$VERSION and yusaozdemir.de/resource-action-operator-catalog:$VERSION.
IMAGE_TAG_BASE ?= yusaozdemir.de/resource-action-operator
...
KIND_CLUSTER ?= resource-action-operator-test-e2e
...
docker-buildx: ## Build and push docker image for the manager for cross-platform support
	...
	- $(CONTAINER_TOOL) buildx create --name resource-action-operator-builder
	$(CONTAINER_TOOL) buildx use resource-action-operator-builder
	- $(CONTAINER_TOOL) buildx build --push --platform=$(PLATFORMS) --tag ${IMG} -f Dockerfile.cross .
	- $(CONTAINER_TOOL) buildx rm resource-action-operator-builder
	rm Dockerfile.cross
...
# A comma-separated list of bundle images (e.g. make catalog-build BUNDLE_IMGS=example.com/operator-bundle:vX.Y.Z,example.com/operator-bundle:vA.B.C).
# These images MUST exist in a registry and be pull-able.
BUNDLE_IMGS ?= $(BUNDLE_IMG)

# The image tag given to the resulting catalog image (e.g. make catalog-build CATALOG_IMG=example.com/operator-catalog:vX.Y.Z).
